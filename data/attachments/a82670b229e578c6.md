...
```
Error: locator.textContent: Error: strict mode violation: locator('.alert, .help-block, [data-test="login-error"]') resolved to 2 elements:
    1) <div aria-live="assertive" data-test="login-error" class="alert alert-danger" _ngcontent-ng-c713820125="">…</div> aka locator('[data-test="login-error"]')
    2) <div class="help-block" _ngcontent-ng-c713820125="">Invalid email or password</div> aka getByText('Invalid email or password')

Call log:
  - waiting for locator('.alert, .help-block, [data-test="login-error"]')
...
```ts
  ...
  14 |         this.errorMessage = page.locator('.alert, .help-block, [data-test="login-error"]'); // Example fallback
  15 |     }
  16 | 
  17 |     async goTo() {
